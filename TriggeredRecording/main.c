...
// #define NUMBER_OF_SAMPLES_IN_TRIGGER_BUFFER (EXTERNAL_SRAM_SIZE_IN_SAMPLES / NUMBER_OF_TRIGGER_BUFFERS)
...
timeSchedule_t defaultTimeSchedule = {
    .startStopPeriods = {
...
static void listenMakeRecording(uint32_t startTime,uint32_t maxDuration, bool enableLED);
...
int main(void) {
    ...
    if (currentTime >= *timeOfNextRecording) {
    	...
    	if (configSettings->useFilter == 1){
    		listenMakeRecording(currentTime,*durationOfNextRecording, enableLED);
    	}else{
...
void AudioMoth_usbApplicationtTimeScheduleRequested(uint32_t messageType, uint8_t *transmitBuffer, uint32_t size){

    memcpy(transmitBuffer + 1,timeSchedule->startStopPeriods, 10*sizeof(uint16_t));
}

void AudioMoth_usbApplicationtTimeScheduleReceived(uint32_t messageType, uint8_t *receiveBuffer, uint8_t *transmitBuffer, uint32_t size){

    /* Copy the USB packet contents to the back-up register data structure location */
    memcpy(timeSchedule->startStopPeriods,receiveBuffer + 1, 10*sizeof(uint16_t));
...
static void startFilter() {
    ...
    float realSampleRate = (float)configSettings->sampleRate / (float)configSettings->sampleRateDivider;
    float targetSampleRate = (float)configSettings->goertzelFreq;
    ...
    while (!recordingCancelled){
            ...
            if (buffersProcessed >= NUMBER_OF_BUFFERS_TO_SKIP) {
                    if (triggerSignal){
                        ...
                        uint32_t currentTime = AudioMoth_getTime();
                        uint32_t chunkDuration = maxDuration - (currentTime - startTime);

                        if (chunkDuration > 5){
                        	buffersProcessed += _makeRecording(startTime, currentTime, chunkDuration, enableLED);
                        } else {
                        	break;
                        }
            ...
            if (maxDuration - (AudioMoth_getTime() - startTime) < 5){
                	break;
            }
...
static void scheduleRecording(uint32_t currentTime, uint32_t *timeOfNextRecording, uint32_t *durationOfNextRecording) {
    ...
    for (uint32_t i = 0; i < configSettings->activeStartStopPeriods; i += 1) {
        ...
        startStopPeriod_t *period = timeSchedule->startStopPeriods + i;
    ...
    startStopPeriod_t *firstPeriod = timeSchedule->startStopPeriods;
