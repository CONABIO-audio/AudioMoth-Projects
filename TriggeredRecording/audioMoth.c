...
#define AM_USB_MSG_TYPE_GET_APP_TIME_SCHEDULE     0x09
#define AM_USB_MSG_TYPE_SET_APP_TIME_SCHEDULE     0x10
...
int dataReceivedCallback(USB_Status_TypeDef status, uint32_t xferred, uint32_t remaining) {
    ...
    switch(receivedMessageType) {
        ...
        case AM_USB_MSG_TYPE_GET_APP_TIME_SCHEDULE:

            /* Requests application time schedule for the device */

            AudioMoth_usbApplicationtTimeScheduleRequested(AM_USB_MSG_TYPE_GET_APP_TIME_SCHEDULE, transmitBuffer, AM_USB_BUFFERSIZE);

            break;

        case AM_USB_MSG_TYPE_SET_APP_TIME_SCHEDULE:

            /* Requests application time schedule for the device */

            AudioMoth_usbApplicationtTimeScheduleReceived(AM_USB_MSG_TYPE_SET_APP_TIME_SCHEDULE,receiveBuffer, transmitBuffer, AM_USB_BUFFERSIZE);
